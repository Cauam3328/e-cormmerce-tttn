...
const login = asyncHandler(async (req, res) => {
  ...
  if (response && (await response.isCorrectPassword(password))) {
    ...
    // tao refreshToken
    const newRefreshrToken = generateRefreshToken(response._id);
...
const refreshAccessToken = asyncHandler(async (req, res) => {
  ...
  // check token có hợp lệ, còn hạn hay k
...
const logout = asyncHandler(async (req, res) => {
  ...
  // Xóa refresh token ở cookie trên trình duyệt
...
const forgotPassword = asyncHandler(async (req, res) => {
  ...
  const html = `Xin vui lòng click vào link dưới đây để thay đổi mật khẩu của bạn.Link này sẽ hết hạn sau 15p kể từ bây giờ. 
  <a href=${process.env.URL_SEVER}/api/user/reset-password/${resetToken}>click here</a>`;
